...
// Bidding analytics API for user and admin dashboard
router.get('/api/bidding-analytics', sessionChecker, async (req, res) => {
  try {
    const userId = req.session.user._id;
    const total = await Projects.countDocuments({ user: userId });
    const success = await Projects.countDocuments({ user: userId, status: 0 });
    const failed = await Projects.countDocuments({ user: userId, status: 1 });
    const winRate = total ? ((success / total) * 100).toFixed(2) : 0;
    
    // Chart data: bids per day (last 7 days)
    let days = [];
    let bidCounts = [];
    for (let i = 6; i >= 0; i--) {
      let day = new Date();
      ...
      let end = new Date(day.getFullYear(), day.getMonth(), day.getDate(), 23, 59, 59, 999);
      days.push(day.toLocaleDateString());
      let count = await Projects.countDocuments({ user: userId, time: { $gte: start, $lte: end } });
      bidCounts.push(count);
    }
    ...
    res.json({ total, success, failed, winRate, chartData });
  } catch (error) {
  ...
  try {
    ...
    if ((response.data.status = "success")) {
      ...
      if (existingUser) {
        ...
        if (
          response2.data.result.avatar_cdn ||
          response2.data.result.avatar_large_cdn
        ) {
...
router.get("/autoBidTurnOn", sessionChecker, async (req, res) => {
  try {
    ...
    // Update user's autoBid status based on the query parameter
      if (userAutoBid === "ON") {
        user.autoBid = true;
        // Only reset timing fields if autobid was previously off or never set
        if (!user.bidStartTime || !user.breakTime || !user.bidEndTime) {
          const now = new Date();
          const timeInterval = parseInt(user.timeInterval) || 2;
          const timeLimit = parseInt(user.timeLimit) || 30;
          user.bidStartTime = now;
          user.bidEndTime = new Date(now.getTime() + timeLimit * 60000);
          user.breakTime = new Date(now.getTime() + timeInterval * 60000);
        }
        // If timing fields exist, keep them so user doesn't have to wait again
      } else {
        user.autoBid = false;
...
router.get("/autoBidTurnOnTimeSetting", sessionChecker, async (req, res) => {
  try {
    ...
    if (userAutoBid === "ON") {
      user.autoBid = true;
      // Initialize timing fields for autobid
      const now = new Date();
      const timeInterval = parseInt(user.timeInterval) || 2;
      const timeLimit = parseInt(user.timeLimit) || 30;
      user.bidStartTime = now;
...
function isIntervalHit(pastDate, timeInterval) {
  ...
  // Check if enough time has passed (current time is past the future time)
...
router.post("/client_stats", sessionChecker, async (req, res) => {
  ...
  await Users.findOneAndUpdate(
    { _id: userId },
    {
      ...
      // Do not reset bidStartTime, bidEndTime, breakTime, or autoBid
    }
  );
...
Keep the response under 250 words and make it sound genuinely interested and professional.`;
              ...
              console.log("🔑 ChatGPT API Key found:", API_KEY ? `${API_KEY.substring(0, 10)}...` : "undefined");
            ...
            if (error.response) {
              console.error("📊 ChatGPT API Response Status:", error.response.status);
              console.error("📊 ChatGPT API Response Data:", JSON.stringify(error.response.data, null, 2));
              
              if (error.response.status === 401) {
                console.error("🔑 Authentication Error: The API key may be invalid, expired, or missing billing information");
                console.error("💡 Please check:");
                console.error("   - API key is valid and active");
                console.error("   - OpenAI account has sufficient credits");
                console.error("   - API key has correct permissions");
              }
            }
            
            // Professional fallback template if AI generation fails
            const clientName = project.fullName || "there";
            const projectSkills = project.jobNames.join(", ");
            const budgetInfo = project.type === "FIXED" ? 
              `$${project.minimumBudget}-$${project.maximumBudget}` : 
              `$${project.minimumBudget}/hour`;
              
            templateContent = `Hi ${clientName}!

I hope you're doing well! I came across your ${project.title} project and I'm really excited about this great opportunity to work with you.
...
I'm really looking forward to discussing your project in more detail and showing you how I can help bring your vision to life. Let's connect and make this project a great success!
...
Best regards`;
          ...
          try {
            ...
            // Check if user has already bid on this project via API
            console.log("🔍 Checking if user has already bid on project:", project.projectid);
            try {
              const existingBidsResponse = await axios.get(
                `https://www.freelancer.com/api/projects/0.1/bids/`,
                {
                  params: {
              ...
              // Check if any bids exist for this project by this user
              if (existingBidsResponse.data?.result?.bids && 
                  existingBidsResponse.data.result.bids.length > 0) {
              ...
              console.log("✅ No existing bid found. Proceeding with bid submission.");
            } catch (bidCheckError) {
              console.log("⚠️ Could not check existing bids, proceeding with caution:", bidCheckError.message);
              // Continue with bid submission even if check fails
            }
            ...
            if (bidError.response) {
              ...
              if (bidError.response.status === 409) {
                const errorMessage = bidError.response.data?.message || '';
                
                if (errorMessage.includes('already bid') || 
                    errorMessage.includes('You have already placed a bid') ||
                    errorMessage.includes('duplicate bid')) {
                  console.log("⚠️ Confirmed: User has already bid on this project");
                ...
                const errorMessage = bidError.response.data?.message || '';
                console.log("❌ Forbidden - user may not have permission to bid on this project");
...
// Test route to reset user bidding settings
router.get("/reset-user-bidding", async (req, res) => {
  try {
    const result = await Users.findOneAndUpdate(
    ...
    res.json({
      success: true,
      message: 'User bidding settings reset successfully!',
      user: {
        autoBid: result.autoBid,
        timeLimit: result.timeLimit,
        timeInterval: result.timeInterval,
        bidsLimit: result.bidsLimit,
        bidStartTime: result.bidStartTime,
        bidEndTime: result.bidEndTime,
...
// Test route to start fresh bidding session
router.get("/start-fresh-session", async (req, res) => {
  try {
    const result = await Users.findOneAndUpdate(
      { email: 'munnumini94@gmail.com' },
      {
        $set: {
          autoBid: true,
          timeLimit: 5,           // 5-minute test session
          timeInterval: 1,        // 1 minute between bids
          bidsLimit: 3,           // Allow 3 bids
          bidStartTime: null,     // Will be set automatically
          bidEndTime: null,       // Will be calculated
          breakTime: null,        // Will be managed automatically
        },
    ...
    if (result) {
      res.json({ 
        success: true, 
        message: "Fresh bidding session started",
        user: {
          _id: result._id,
          autoBid: result.autoBid,
          timeLimit: result.timeLimit,
